...
**Overlays** are driven by two observers with separate lists — `OVERLAYS` (focus-in, focus restore, `role=dialog`, Escape) and `OVER` (scroll lock, back-button history, focus containment). **An overlay needs to be in both.** `#cfmModal` was in `OVERLAYS` only and, being built on first use, was missed by both observers, because each registered with one `querySelector` at load. Register through `HMOverlayWatch(sel, attach)` instead — it waits for a lazily built overlay and runs `attach` immediately on arrival, since such an overlay is normally created and opened in the same task.

`#tourWrap` is in `OVER` only, and that one is deliberate: the tour is a coach-mark with its own Skip/Next, not a modal dialog, so it gets the scroll lock and back-button close without `role=dialog` or a focus grab.
